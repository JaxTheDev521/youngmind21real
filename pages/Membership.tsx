...
const Membership: React.FC = () => {
  ...
  return (
    <div className="bg-neutral-light py-20 px-4 min-h-screen">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16 animate-fade-in">
          <h1 className="text-5xl md:text-6xl font-extrabold font-lexend mb-6 text-midnight tracking-tight">Choose Your Path</h1>
          <p className="text-gray-600 max-w-2xl mx-auto text-lg leading-relaxed">Every child can lead. Pick a membership that fits your goals and start building your future today.</p>
        ...
        <div className="grid md:grid-cols-2 gap-10 items-stretch max-w-5xl mx-auto">
          {plans.map((plan, i) => (
            <div key={i} className={`rounded-[3rem] p-10 md:p-12 border transition-all duration-500 flex flex-col hover:-translate-y-2 animate-fade-in ${plan.highlight
              ? 'glass-dark text-white border-white/10 shadow-2xl scale-105 z-10'
              : 'glass text-midnight border-white/50 shadow-xl hover:shadow-orange-glow/10'}`}
              style={{ animationDelay: `${i * 150}ms` }}
            >
              <div className="mb-10">
                <div className="flex justify-between items-start mb-4">
                  <h3 className="text-3xl font-bold font-lexend">{plan.name}</h3>
                  {plan.highlight && <span className="px-4 py-1.5 bg-highlight-yellow text-midnight rounded-full text-xs font-bold uppercase tracking-widest animate-pulse">Most Popular</span>}
                </div>
                <div className="flex items-baseline gap-2">
                  <span className="text-6xl font-black">{plan.price}</span>
                  <span className="text-base opacity-60 font-medium">{plan.period}</span>
                </div>
                <p className="mt-6 opacity-80 leading-relaxed text-lg font-medium">{plan.description}</p>
              </div>
              ...
              <Link
                to="/onboarding"
                className={`block text-center py-5 rounded-2xl font-bold transition-all transform active:scale-95 shadow-2xl text-lg tracking-wide ${plan.highlight
